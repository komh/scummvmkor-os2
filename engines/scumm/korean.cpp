...
void loadKoreanStrings()
{
...
#if defined(YOPY)
    Common::File::addDefaultDirectory("/usr/share/scummvmk/sub");
#else
    Common::File::addDefaultDirectory("sub");
#endif

	strcpy(fname, _gameID);
	strcat(fname, ".dat");

	if(!fp.open(fname)) {
		printf("WARNING: Cannot load Korean V2 subtitle!, %s\n", fname);
		return;
	}
	...
	for(int i = 0; i < _numKLines; i++) {
		...
		if (len > 1000)
			printf("_KBuffer[%d]:%x, len=%d\n", i, (int)_KBuffer[i], strlen(buf));
		if(strlen(buf)) {
	...
	printf("Korean subtitle file loaded -- total %d lines, %d bytes\n", _numKLines, _KBuffer[_numKLines-1]-_KBuffer[0]+len);
	//printf("Used %d bytes\n", _KBuffer[_numKLines-1]-_KBuffer[0]+len);
	delete buf;
